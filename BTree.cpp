...
iterator BTree::insert(StringView strView){
    auto proxy = detail::PageProxy::fromPagePtr(root_);
    auto ret = proxy->insert
        (strView,tupleCmp_,keyCmp_,retrieveKey_,pageAllocator_);
    return iterator{ret.first,pageAllocator_,ret.second};
